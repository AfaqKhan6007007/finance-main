...
interface FormData {
    ...
    columns: string[]; // Change from never[] to string[]
}
...
export default function KanbanCreateModal({ handleShowKanbanBoard, onKanbanBoardAdded }: KanbanCreateModalProps) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            const response = await axios.post('/api/user/kanbanView', formData);
            if (response.data.success){
                showSuccessToast("Kanban board created successfully!");
                handleShowKanbanBoard(); // Close modal on success
                    ...
                    <div className="space-y-2">
                        ...
                        >
                            ...
                            <SelectContent className="min-w-[var(--radix-select-trigger-width)]">
                                <SelectItem value="rootType" className="h-12 px-4 text-base flex items-center">
                                    Root Type
                                </SelectItem>
                                <SelectItem value="reportType" className="h-12 px-4 text-base flex items-center">
                                    Report Type
                                </SelectItem>
                                <SelectItem value="accountType" className="h-12 px-4 text-base flex items-center">
                                    Account Type
                                </SelectItem>
                                <SelectItem value="frozen" className="h-12 px-4 text-base flex items-center">
                                    Frozen
                                </SelectItem>
                                <SelectItem value="balanceMustBe" className="h-12 px-4 text-base flex items-center">
                                    Balance Must Be
                                </SelectItem>
                            </SelectContent>
                        </Select>
